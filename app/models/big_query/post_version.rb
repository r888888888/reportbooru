module BigQuery
  class PostVersion < Base
    ...
    def aggregate_missing_tags(user_id, post_ids)
      get_two query("select added_tag, count(*) from [danbooru_#{Rails.env}.post_versions_flat_part] where _partitiontime >= timestamp('#{part_s}') and updater_id <> #{user_id} and post_id in (#{post_ids.join(', ')}) and version > 1 and updated_at >= '#{date_s}' and added_tag is not null group by added_tag having count(*) > 5 order by count(*) desc")
    end
